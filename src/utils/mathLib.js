...
export function I1(alpha, IX, IY, IXY) {
  ...
  return (
    IX * Math.pow(cos(alpha), 2) -
    2 * IXY * sin(alpha) * cos(alpha) +
    IY * Math.pow(sin(alpha), 2)
  );
}
...
export function I2(alpha, IX, IY, IXY) {
  ...
  // let cos = this.cos_alpha(alpha)
  return (
    IX * sin(alpha) * cos(alpha) +
    IXY * Math.pow(cos(alpha), 2) +
    IXY * Math.pow(sin(alpha), 2) -
    IY ** sin(alpha) * cos(alpha)
  );
}

export function decimal(num, deci){
  if(num < 0){
    return parseFloat(Math.round(num * 100) / 100).toFixed(deci)
  }
  else{
...
export function deciToFrac(val) {
  ...
  if (inch16 > 0) {
    switch (Math.round(inch16)) {
      ...
      default:
        return (
          ft + " ft " + Math.trunc(inch) + " " + Math.round(inch16) + "/16 in"
        );
        break;
    }
  } else {
