...
export default class CapacityLU extends Steel {
  ...
  Zt1() {
    return (
      (this.LU.LL - this.LU.Y) * this.sin_alpha() - this.LU.X * this.cos_alpha()
    );
  }
  Zt2() {
  ...
  //-------------------------------------------------
  FlexureCompactness() {
    return this.LU.LL / this.LU.T < 0.54 * Math.pow(this.E / this.Fy, 0.5)
      ? "Compact"
      : this.LU.LL / this.LU.T <= 0.91 * Math.pow(this.E / this.Fy, 0.5)
      ? "Non-Compact"
      : "Slender";
  } //FlangeFlexure

  CompressionCompactness() {
    return this.LU.LL / this.LU.T < 0.45 * Math.pow(this.E / this.Fy, 0.5)
      ? "Compact"
      : this.LU.LL / this.LU.T <= 0.91 * Math.pow(this.E / this.Fy, 0.5)
      ? "Non-Compact"
      : "Slender";
  ...
  Mllbx() {
    ...
    if (this.FlexureCompactness() == "Non-Compact") {
      return (
        ...
        Sc *
        (2.43 -
          1.72 * (this.LU.LL / this.LU.T) * Math.pow(this.Fy / this.E, 0.5))
      );
    } else {
      return this.Fcrz() * Sc;
    }
  ...
  Me() {
    if (this.LU.LL == this.LU.SL) {
      return (
        (0.46 *
          this.E *
          Math.pow(this.LU.LL, 2) *
          Math.pow(this.LU.T, 2) *
          this.cb) /
        this.Lu
      );
    } else {
      ...
      let m2 = Math.pow(
        Math.pow(b, 2) +
          0.052 * Math.pow((this.Lu * this.LU.T) / this.LU.RZ, 2),
        0.5
      );
  ...
  Mltbx() {
    if (this.Me() <= this.Mnyx()) {
      ...
      return Math.min(
        (0.92 - 1.17 * Math.pow(this.Me() / this.Mnyx(), 0.5)) * this.Me(),
        1.5 * this.Mnyx()
      );
    }
  } //Mltbx
  Mltby() {
    if (this.Me() <= this.Mnyy()) {
      ...
      return Math.min(
        (0.92 - 1.17 * Math.pow(this.Me() / this.Mnyy(), 0.5)) * this.Me(),
        1.5 * this.Mnyy()
      );
    }
  } //Mltby
  ...
  fMnx() {
    return (
      (ArrayMin(this.Mnyx(), this.Mltbx(), this.Mllbx()) * this.phiF()) / 12
    );
  } //fMnx
  fMny() {
    return (
      (ArrayMin(this.Mnyy(), this.Mltby(), this.Mllby()) * this.phiF()) / 12
    );
  } //fMny
  flexureParams() {
    return {
