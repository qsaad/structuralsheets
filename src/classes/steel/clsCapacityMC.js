...
export default class CapacityMC extends Steel {
  ...
  FlangeFlexure() {
    return this.BF_2TF() < 0.38 * this.k()
      ? "Compact"
      : this.BF_2TF() < 1 * this.k()
      ? "Non-Compact"
      : "Slender";
  } //FlangeFlexure

  WebFlexure() {
    return this.H_TW() < 3.76 * this.k()
      ? "Compact"
      : this.H_TW() < 5.7 * this.k()
      ? "Non-Compact"
      : "Slender";
  } //WwebFlexure
  ...
  Lr() {
    ...
    let v3 =
      6.76 *
      Math.pow(
        (0.7 * this.Fy * this.CH.SX * this.HO()) /
          (this.E * this.CH.J * this.c()),
        2
      );
  ...
  Fcr() {
    var v1 =
      (this.Cb * Math.pow(Math.PI, 2) * this.E) /
      Math.pow(this.Lu / this.RTS(), 2);
    var v2 =
      ((0.078 * (this.CH.J * this.c())) / (this.CH.SX * this.HO())) *
      Math.pow(this.Lu / this.RTS(), 2);
  ...
  Mltb() {
    if (this.Lu <= this.Lp()) {
      ...
      var v1 =
        this.Cb *
        (this.Mpx() -
          (this.Mpx() - 0.7 * this.Fy * this.CH.SX) *
            ((this.Lu - this.Lp()) / (this.Lr() - this.Lp())));
  ...
  Mflb() {
    ...
    if (this.FlangeFlexure() == "Non-Compact") {
      return (
        this.Mpx() -
        (this.Mpx() - 0.7 * this.Fy * this.CH.SX()) *
          ((this.BF_2TF() - this.lpf()) / (this.lrf() - this.lpf()))
      );
    } else {
  ...
  Mflby() {
    ...
    if (this.FlangeFlexure() == "Non-Compact") {
      return (
        this.Mpy() -
        (this.Mpy() - 0.7 * this.Fy * this.CH.SY) *
          ((this.lf() - this.lpf()) / (this.lrf() - this.lpf()))
      );
    } else {
  ...
  Fez() {
    return (
      ((Math.pow(Math.PI, 2) * this.E * this.CH.CW) /
        Math.pow(this.Ky * this.Ly, 2) +
        this.G * this.CH.J) *
      (1 / (this.CH.A * this.CH.RO * this.CH.RO))
    );
  } //Fez

  Fet() {
    return (
      ((this.Fey() + this.Fez()) / (2 * this.CH.H)) *
      (1 -
        Math.pow(
          1 -
            (4 * this.Fey() * this.Fez() * this.CH.H) /
              Math.pow(this.Fey() + this.Fez(), 2),
          0.5
        ))
    );
  } //Fet
  ...
  Qa() {
    ...
    if (this.H_TW() >= 1.49 * Math.pow(this.E / f, 0.5)) {
      var be =
        ...
        this.CH.TW *
        Math.pow(this.E / f) *
        (1 - (0.34 / this.BF_2TF()) * Math.pow(this.E / f, 0.5));
