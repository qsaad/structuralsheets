...
export default class CapacityHSR extends Steel {
  ...
  //-----------------------------------------------------------
  WallFlexure() {
    return this.HSR.D_T < (0.07 * this.E) / this.Fy
      ? "Compact"
      : this.HSR.D_T < (0.31 * this.E) / this.Fy
      ? "Non-Compact"
      : "Slender";
  } //WallFlexure
  ...
  Fet() {
    return (
      ((Math.pow(Math.PI, 2) * this.E * this.HSR.C) /
        Math.pow(this.Kx * this.Ly, 2) +
        this.G * this.HSR.J) *
      (1 / (this.HSR.IX + this.HSR.IY))
    );
  } //Fet
